...
* **Pretraining**:
  ...
  * Save pretrained weights to `checkpoints/rl_champion_v2.pt`.
...
* **Champion Evaluation Gate**:
  * Keep the training model (`engine/rl_active_v2.pt`) separate from the validated champion (`checkpoints/rl_champion_v2.pt`).
