# agents/rl/v2/encoder_v2.py
"""Upgraded state encoder (Encoder V2) for RL Agent 2.0.

Supports variable player counts (3 to 6) by allocating feature space for
up to 6 players and padding empty seats with zeros. Feature vector size: 459.
"""
...
def encode_state_v2(state: EngineState, player_id: int) -> np.ndarray:
    """Encodes EngineState into a flat float32 NumPy array of size 459.

    Pads features for any player count less than 6 with zeros.
    """
    ...
    # 6 players * 2 features (half_points, rounds_won)
    ...
    # Concatenate all into a single flat vector of size 459
