...
def project_determinization(viewer_id: int, tracker: CardTracker, state: EngineState, rng: random.Random) -> EngineState:
    ...
    backtrack_steps = 0
    max_steps = 2000  # Safe threshold to prevent CPU hangs on highly constrained hands

    def backtrack(card_idx: int) -> bool:
        nonlocal backtrack_steps
        backtrack_steps += 1
        if backtrack_steps > max_steps:
            return False
