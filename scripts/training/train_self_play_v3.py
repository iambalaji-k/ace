# scripts/training/train_self_play_v3.py
...
class LeagueMCTSAgent(MCTSAgent):
    """MCTS Agent configured with 200 iterations and 0.5s time limit for training."""
    def __init__(self, player_id: int, num_players: int, seed=None):
        super().__init__(player_id, seed=seed, max_iterations=200, time_limit=0.50)
...
def sample_opponent(active_model: AceNetV2, checkpoints_dir: str, player_id: int, seed: int, num_players: int) -> BaseAgent:
    ...
    elif r < 0.95:  # MCTS (20% - increased from 10%)
        return LeagueMCTSAgent(player_id=player_id, num_players=num_players, seed=seed)
    else:  # RL v1.0 / Random (5% - decreased from 15%)
...
def compute_normalized_potential(state: EngineState, learning_player_id: int) -> Tuple[float, float, float]:
    ...
    # 98% Hand clearances priority vs 2% Knowledge acquisition ratio (info_hat weight reduced to 10% of original)
    ...
    while not AceEngine.is_terminal(state):
        ...
        for event in events:
            if event.event_type == "ROUND_ENDED":
                is_draw = event.payload.get("is_draw", False)
                if is_draw:
                    round_reward = 0.0
                else:
                    loser_id = event.payload["loser_id"]
                    winner_ids = list(event.payload["winner_ids"])
                ...
                if trajectories:
                    ...
                    trajectories[-1]['reward'] += accumulated_reward + final_shaping
...
SHAPING_COEFF = 0.05  # Very minimal shaping coefficient (5% of round outcomes)
GAMMA = 0.99          # Discount factor

def train_self_play_v3(epochs: int = 200, matches_per_epoch: int = 40):
    ...
    champion_path = "checkpoints/rl_champion_v3.pt"
    active_path = "checkpoints/rl_active_v3.pt"
    resume_path = "checkpoints/train_resume_v3.pt"
    ...
    if start_epoch == 1:
        initializer_path = "checkpoints/rl_champion_v2.pt"  # Use v2 imitation model as initial weights
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        decay_rate = (3e-5 / 3e-4) ** (1 / 199)  # 0.988553
